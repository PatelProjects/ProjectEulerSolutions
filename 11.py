...
for y in range(len(M)):
    for x in range(len(M[y])):
        if x < len(M[y])-3:
            number = M[y][x] * M[y][x+1] * M[y][x+2] * M[y][x+3]
            if number > maX:
                maX = number
            
        if y < len(M)-3:
            number = M[y][x] * M[y+1][x] * M[y+2][x] * M[y+3][x]
            if number > maX:
                maX = number

for x in range(len(M[0])-1,3,-1):
    for y in range(len(M)-1,3,-1):
         
        x2 = len(M[0]) -x
        y2 = len(M) - y
         
        if x2 < len(M[0])-3:
            number =  M[x2][y] * M[x2+1][y-1] * M[x2+2][y-2] * M[x2+3][y-3]
            if number > maX:
                    maX = number
                     
        if y2 < len(M) -3:
            number =  M[x][y2] * M[x-1][y2-1] * M[x-2][y2-2] * M[x-3][y2-3]
